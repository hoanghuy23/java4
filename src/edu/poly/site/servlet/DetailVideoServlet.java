 ...
 */
@WebServlet({"/DetailVideoServlet", "/DetailVideoServlet/insert", "/DetailVideoServlet/delete", "/DetailVideoServlet/share"})
public class DetailVideoServlet extends HttpServlet {
	...
	private void share(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		String videoId = request.getParameter("videoId");
		response.sendRedirect("http://localhost:8080/WebVideo/ShareVideoServlet?videoId=" + videoId);
	}
	...
	private void unlike(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			response.sendRedirect("http://localhost:8080/WebVideo/DetailVideoServlet?videoId=" + videoId);
		} catch (Exception e) {
			e.printStackTrace();
		}
